# Traffic Anomaly Detection with TP-FDS
This repository contains traffic data with anomalies being labeled by 20 different experts using TP-FDS. 
A Python code with relvant explanatins `TPFDS-ABOD.py` are also included that applies Fast-ABOD to traffic data benefiting from the TP-FDS. Other anomaly detection methods also can be embedded in this code for comparison purposes (benefiting from the [PyOD library](https://pyod.readthedocs.io/en/latest/)).
